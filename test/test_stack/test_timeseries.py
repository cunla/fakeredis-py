...
def test_add(r: redis.Redis):
    ...
    assert_resp_response(
        r, 10, info.get("retention_msecs"), info.get("retentionTime")
    )
    ...
    info = r.ts().info("time-serie-1")
    assert_resp_response(r, 128, info.get("chunk_size"), info.get("chunkSize"))
...
def test_incrby_decrby(r: redis.Redis):
    ...
    assert_resp_response(r, r.ts().get(2), (5, 1.5), [5, 1.5])
    assert r.ts().incrby(2, 2.25, timestamp=7)
    assert_resp_response(r, r.ts().get(2), (7, 3.75), [7, 3.75])
    assert r.ts().decrby(2, 1.5, timestamp=15)
    assert_resp_response(r, r.ts().get(2), (15, 2.25), [15, 2.25])
...
def test_del_range(r: redis.Redis):
    ...
    assert_resp_response(r, r.ts().range(1, 22, 22), [(22, 1.0)], [[22, 1.0]])
...
def test_range_advanced(r: redis.Redis):
    ...
    assert_resp_response(r, res, [(0, 10.0), (10, 1.0)], [[0, 10.0], [10, 1.0]])
    res = r.ts().range(
        1, 0, 10, aggregation_type="count", bucket_size_msec=10, align=5
    )
    assert_resp_response(r, res, [(0, 5.0), (5, 6.0)], [[0, 5.0], [5, 6.0]])
    res = r.ts().range(1, 0, 10, aggregation_type="twa", bucket_size_msec=10)
    assert_resp_response(r, res, [(0, 2.55), (10, 3.0)], [[0, 2.55], [10, 3.0]])
...
def test_range_latest(r: redis.Redis):
    ...
    assert_resp_response(r, timeseries.range("t2", 0, 10), [(0, 4.0)], [[0, 4.0]])
    res = timeseries.range("t2", 0, 10, latest=True)
    assert_resp_response(r, res, [(0, 4.0), (10, 8.0)], [[0, 4.0], [10, 8.0]])
    assert_resp_response(
        r, timeseries.range("t2", 0, 9, latest=True), [(0, 4.0)], [[0, 4.0]]
    )
...
def test_rev_range(r: redis.Redis):
    ...
    assert_resp_response(
        r,
        r.ts().revrange(1, 0, 10, aggregation_type="twa", bucket_size_msec=10),
        [(10, 3.0), (0, 2.55)],
        [[10, 3.0], [0, 2.55]],
    )
...
def test_revrange_latest(r: redis.Redis):
    ...
    assert_resp_response(r, res, [(0, 4.0)], [[0, 4.0]])
    res = timeseries.revrange("t2", 0, 10, latest=True)
    assert_resp_response(r, res, [(10, 8.0), (0, 4.0)], [[10, 8.0], [0, 4.0]])
