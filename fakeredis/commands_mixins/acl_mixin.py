...
from fakeredis._helpers import SimpleError, OK, casematch, SimpleString, client_info_as_bytes
...
class AclCommandsMixin:
    ...
    def _auth(self, *args: bytes) -> bytes:
        ...
        if len(args) >= 1 and self._check_user_password(username, password):
            ...
            return OK
        self._acl.add_log_record(b"auth", b"auth", b"AUTH", username, client_info_as_bytes(self._client_info))
